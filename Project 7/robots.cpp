...
void Player::moveOrAttack(int dir)
{
    ...
    switch (dir)
    {
        case UP:
            ...
            {
                m_arena->attackRobotAt(m_row-1, m_col, UP);
            }
            else if (m_row-1 >= 1)
                m_row--;
            break;
        case DOWN:
            if (m_arena->nRobotsAt(m_row+1, m_col) > 0)
            {
                m_arena->attackRobotAt(m_row+1, m_col, DOWN);
            }
        ...
        case LEFT:
            ...
            {
                m_arena->attackRobotAt(m_row, m_col-1, LEFT);
            }
            else if (m_col-1 >= 1)
                m_col--;;
            break;
        case RIGHT:
            if (m_arena->nRobotsAt(m_row, m_col+1) > 0)
            {
                m_arena->attackRobotAt(m_row, m_col+1,RIGHT);
            }
...
bool Arena::moveRobots()
{
    ...
    {
        m_robots[k]->move();
        if (nRobotsAt(m_player->row(), m_player->col()) > 0)
            m_player->setDead();
    }
